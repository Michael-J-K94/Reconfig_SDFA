...
class StoLinear_Int(nn.Module):
    def __init__(self, in_features, out_features, fb_features, FL=10, FL_use = 6, IL = 2, bias=False):
    ...
    def forward(self, input):
        ...
        return sample(out.float() / self.offset_)
    ...
    def record(self, input):
        ...
        :return:
        """
        fb = torch.matmul(input, (self.fb_weight/self.weight_use_offset).float().floor()) 
        self.fb_f = fb.clone()
        return
    ...
    def backward(self, input):
        ...
        self.weight = self.weight + dw#(dw.float()/8).round().int()
...
class StoOutput_Int(StoLinear_Int):
    ...
    def backward(self, input):
        ...
        self.weight = self.weight + dw#(dw.float()/32).round().int()
        if self.bias_use:
